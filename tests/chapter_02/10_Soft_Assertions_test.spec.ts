...
test.describe('Soft Assertions in Playwright', () => {
  ...
  test('URL, Title, Text and Disabled assertions', async({ page }) => {
    // visible, editable, enabled, empty
    const search = page.getByRole('combobox', {name: 'Search with DuckDuckGo'});
    ...
    await test.step('Text assertion - expect(page.locator()).toHaveText("Text")', async () => {
      await expect.soft(page.locator('h1 span[role="text"]').nth(1)).toHaveText('Playwright Typescript by Testers Talk☑️')

...
import putAPIRequest from '../../test-data/api_requests/PUT_API_Requeest.json';
test.use({
  baseURL: process.env.BASE_API_URL,
})

test('Create PUT API request using Faker library only ', async({request}) =>{
  ...
  console.log("expect(putAPIresponse.status()).toBe(200)");
  expect(putAPIresponse.statusText()).toBe('OK');
  console.log("expect(putAPIresponse.statusText()).toBe('OK')");

  const putAPIJsonResponse = await putAPIresponse.json();
  console.log("const putAPIJsonResponse = await putAPIresponse.json()");

  console.log("\nPUT API Response Headers:")
  console.log("putAPIresponse.headers(): ", putAPIresponse.headers()); //response header 

  console.log("\nPUT API Response Body:")
  console.log("👉🏽 putAPIJsonResponse: ", putAPIJsonResponse); //response body
})

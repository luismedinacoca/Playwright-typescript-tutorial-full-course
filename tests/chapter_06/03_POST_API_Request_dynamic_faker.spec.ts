...
test('Create POST API request using Faker library', async({request}) =>{
  ...
  console.log("❄️ checkin: ", typeof checkin, typeof checkout);
...
test('Create POST API request using Faker library only ', async({request}) =>{
  const firstname = faker.person.firstName(); 
  const lastname = faker.person.lastName(); 
  const totalprice = faker.number.int({min: 1000, max: 10000});
  const depositepaid = faker.datatype.boolean();
  ...
  //const jsonPostAPIrequest = JSON.parse(postAPIrequest);
  console.log('👉🏽 jsonPostAPIrequest: ', postAPIrequest);
  console.log('')

  const jsonPostAPIrequest = postAPIrequest
  // Create POST API Request
  const postAPIresponse = await request.post(`/booking`, { data: jsonPostAPIrequest });
  console.log("postAPIresponse: ", postAPIresponse); //response header 
